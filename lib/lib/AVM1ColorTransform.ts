...
export function fromAVM1Object(v: AVM1Object): AVM1ColorTransform {
	...
	let off_a: number = v.alGet('alphaOffset');
	if (off_a == null) {
		off_a = v.alGet('ab');
	}
	let mul_a: number = v.alGet('alphaMultiplier');
	if (mul_a == null) {
		mul_a = v.alGet('aa');
		if (mul_a != null) {
			mul_a = mul_a / 100;
	...
	let off_r: number = v.alGet('redOffset');
	if (off_r == null) {
		off_r = v.alGet('rb');
	}
	let mul_r: number = v.alGet('redMultiplier');
	if (mul_r == null) {
		mul_r = v.alGet('ra');
		if (mul_r != null) {
			mul_r = mul_r / 100;
	...
	let off_g: number = v.alGet('greenOffset');
	if (off_g == null) {
		off_g = v.alGet('gb');
	}
	let mul_g: number = v.alGet('greenMultiplier');
	if (mul_g == null) {
		mul_g = v.alGet('ga');
		if (mul_g != null) {
			mul_g = mul_g / 100;
	...
	let off_b: number = v.alGet('blueOffset');
	if (off_b == null) {
		off_b = v.alGet('bb');
	}
	let mul_b: number = v.alGet('blueMultiplier');
	if (mul_b == null) {
		mul_b = v.alGet('ba');
		if (mul_b != null) {
			mul_b = mul_b / 100;
...
export class AVM1ColorTransform extends AVM1Object {
	...
	public setRgb(rgb: number) {
		const argb: number[] = ColorUtils.float32ColorToARGB(rgb);
...
export class AVM1ColorTransformFunction extends AVM1Function {
	...
	alConstruct(args?: any[]): AVM1Object {
		const obj = Object.create(AVM1ColorTransform.prototype);
		args = args || [];
