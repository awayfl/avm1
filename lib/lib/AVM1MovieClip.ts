...
export class AVM1MovieClip extends AVM1SymbolBase<MovieClip> implements IMovieClipAdapter {
	...
	public beginGradientFill(fillType: GradientType, colors: AVM1Object, alphas: AVM1Object,
		...
		var colorsNative = AVM1ArrayNative.mapToJSArray(colors, (item) => alToInt32(this.context, item));
		var alphasNative = AVM1ArrayNative.mapToJSArray(alphas, (item) => alToNumber(this.context, item) / 100.0);
	...
	public startDrag(lock?: boolean, left?: number, top?: number, right?: number, bottom?: number): void {
		...
		if (!this.isDragging) {
			...
			MouseManager.getInstance(AVM1Stage.stage.scene.renderer.renderGroup.pickGroup).startDragObject(this.adaptee);
	...
	public stopDrag(e = null) {
		...
		MouseManager.getInstance(AVM1Stage.stage.scene.renderer.renderGroup.pickGroup).stopDragObject();
