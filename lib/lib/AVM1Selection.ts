...
export class AVM1Selection extends AVM1Object {
    ...
    getBeginIndex():number{
        var objectinFocus:DisplayObject=<DisplayObject>MouseManager.getInstance(PickGroup.getInstance((<AVM1Stage>this.context.globals.Stage).avmStage.scene.renderer.view)).getFocus();
    ...
    getEndIndex():number{
        var objectinFocus:DisplayObject=<DisplayObject>MouseManager.getInstance(PickGroup.getInstance((<AVM1Stage>this.context.globals.Stage).avmStage.scene.view)).getFocus();
        if(objectinFocus && objectinFocus.isAsset(TextField)){
